-- Complete customer-facing rental email lifecycle.
--
-- Adds useful order details to every message, sends schedule confirmations with
-- the actual delivery/pickup window, and queues idempotent day-of reminders.
-- pg_cron runs the reminder/outbox sweep so delivery does not depend on a staff
-- browser being open. All event keys are deterministic to prevent duplicates.
...
DECLARE
  ...
  v_monthly_rate NUMERIC; v_payment_status TEXT; v_payment_preference TEXT;
  v_details_html TEXT; v_details_text TEXT;
BEGIN
  SELECT c.email, o.order_no, o.start_date, o.end_date, o.monthly_rate,
         o.payment_status, o.payment_preference,
         concat_ws(', ', NULLIF(btrim(o.address_line1), ''),
           NULLIF(btrim(o.address_city), ''),
           NULLIF(btrim(concat_ws(' ', o.address_state, o.address_zip)), ''))
    INTO v_email, v_no, v_start_date, v_end_date, v_monthly_rate,
         v_payment_status, v_payment_preference, v_address_text
  ...
  SELECT
    ...
    FROM (
      SELECT ei.name, sum(GREATEST(li.quantity, 1))::int AS quantity
  ...
  v_address_text := NULLIF(btrim(v_address_text), '');
  v_address_html := public.email_html_escape(COALESCE(v_address_text, 'To be confirmed'));
  ...
  v_details_html := '<div style="background:#eff6ff;border-radius:10px;padding:14px;font-size:14px;line-height:1.65">'
    ...
    || CASE WHEN v_items_html IS NOT NULL
      THEN '<div style="margin-top:8px">' || v_items_html || '</div>' ELSE '' END
    ...
    || CASE
      WHEN v_payment_status = 'refunded' THEN 'Refund initiated'
      WHEN v_payment_status = 'paid' THEN 'Paid online'
      WHEN v_payment_preference = 'online' THEN 'Online payment pending'
      ELSE 'Pay in store'
    END || '</div></div>';

  v_details_text := 'Order #' || v_no
    || CASE WHEN v_items_text IS NOT NULL THEN E'\n' || v_items_text ELSE '' END
    || E'\nService address: ' || COALESCE(v_address_text, 'To be confirmed')
    || CASE WHEN v_start_date IS NOT NULL
...
-- A storefront order row is created just before its line items. Refresh the
-- still-pending receipt after each line mutation so the final transactional
-- email contains the complete equipment list rather than an empty placeholder.
...
BEGIN
  ...
  IF v_order.payment_status = 'paid' THEN
    PERFORM public.queue_customer_status_email(
      ...
      'We received your first month rental payment. Our team is reviewing the equipment and delivery details below.');
  ELSE
    PERFORM public.queue_customer_status_email(
      v_order.id, 'request_received',
      'Rental request #' || v_order.order_no || ' received',
      'We received your rental request',
      'Our team is reviewing the equipment and delivery details below. You chose to pay in store, so no online payment was taken.');
  END IF;
  RETURN COALESCE(NEW, OLD);
...
DROP TRIGGER IF EXISTS trg_refresh_request_details_email ON public.rental_line_items;
CREATE TRIGGER trg_refresh_request_details_email
  AFTER INSERT OR DELETE OR UPDATE OF equipment_item_id, quantity,
    monthly_rate, sale_price
  ON public.rental_line_items
  FOR EACH ROW EXECUTE FUNCTION public.refresh_request_details_email();

-- Scheduling details belong to the dispatch leg rather than the order status.
-- Include date/window/address and send another message if staff reschedule it.
...
BEGIN
  ...
  v_when := to_char(NEW.scheduled_date, 'FMDay, FMMonth FMDD, YYYY')
    || CASE
      ...
      WHEN NEW.window_start IS NOT NULL
        THEN ' between ' || to_char(NEW.window_start, 'FMHH12:MI AM')
          || CASE WHEN NEW.window_end IS NOT NULL
            THEN ' and ' || to_char(NEW.window_end, 'FMHH12:MI AM') ELSE '' END
      ELSE ''
    END;
  ...
  PERFORM public.queue_customer_status_email(
    ...
    v_kind || ' scheduled',
    v_kind || ' is scheduled for ' || v_when || '.'
...
DROP TRIGGER IF EXISTS trg_queue_delivery_schedule_email ON public.deliveries;
CREATE TRIGGER trg_queue_delivery_schedule_email
  AFTER INSERT OR UPDATE OF status, driver_id, scheduled_date,
    window_start, window_end, window_label, address_line1, address_city,
    address_state, address_zip
  ON public.deliveries
  FOR EACH ROW EXECUTE FUNCTION public.queue_delivery_schedule_email();

-- Order-status messages cover the decision and completion milestones. The
-- dispatch trigger above owns schedule confirmations so dates never say TBD.
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.status = 'requested' THEN
    ...
    v_message := 'Our team is reviewing the equipment and delivery details below. You chose to pay in store, so no online payment was taken.';
  ELSIF TG_OP = 'UPDATE' AND OLD.status = 'pending_payment'
    ...
    v_message := 'We received your first month rental payment. Our team is reviewing the equipment and delivery details below.';
  ELSIF NEW.status = 'open' THEN
    v_event := 'request_accepted';
    v_subject := 'Rental request #' || NEW.order_no || ' approved';
    v_heading := 'Your rental request was approved';
    v_message := 'Your equipment has been reserved. We will send the delivery date, time window, and location as soon as the route is scheduled.';
  ...
  ELSIF NEW.status = 'closed' THEN
    ...
    v_message := CASE WHEN NEW.order_type = 'rental'
      THEN 'Pickup is complete and the rental window is now closed. Thank you for choosing Cottage Surgical.'
  ...
  ELSIF NEW.status = 'cancelled' THEN
    ...
    v_subject := 'Decision for rental request #' || NEW.order_no;
    ...
    v_message := CASE WHEN NEW.payment_status = 'refunded'
      THEN 'We are unable to fulfill this request, so no delivery or pickup will be scheduled. We initiated a full refund to your original Stripe payment method; your bank may take several business days to post it.'
      ELSE 'We are unable to fulfill this request, so no delivery or pickup will be scheduled. No online payment was taken. Please call us if you would like help with another option.' END;
  ELSE
    RETURN NEW;
  END IF;
...
-- Queue one delivery or return reminder on the morning of each scheduled leg.
-- If a same-day stop is scheduled after 8 AM, the next sweep sends it.
      ...
      WHEN v_leg.window_start IS NOT NULL
        THEN to_char(v_leg.window_start, 'FMHH12:MI AM')
          || CASE WHEN v_leg.window_end IS NOT NULL
            THEN '–' || to_char(v_leg.window_end, 'FMHH12:MI AM') ELSE '' END
      ELSE 'today'
    END;
    v_id := public.queue_customer_status_email(
      v_leg.order_id,
      v_leg.leg_type || '_' || v_leg.id::text || '_day_of_' || v_leg.scheduled_date::text,
      initcap(v_kind) || ' reminder - order #' || v_leg.order_no,
      'Your ' || v_kind || ' is today',
      'This is a reminder that your Cottage Surgical ' || v_kind || ' is scheduled for ' || v_when || '.'
        || CASE WHEN v_leg.leg_type = 'pickup'
          THEN E'\nPlease have the rental equipment ready and accessible for the driver.' ELSE '' END
    );
...
CREATE OR REPLACE FUNCTION public.run_customer_email_schedule()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE v_reminders JSONB; v_delivery JSONB;
BEGIN
  v_reminders := public.queue_due_customer_reminders();
  v_delivery := public.process_customer_email_outbox(10);
  RETURN jsonb_build_object('ok', true, 'reminders', v_reminders, 'delivery', v_delivery);
END;
$$;
REVOKE ALL ON FUNCTION public.run_customer_email_schedule() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.run_customer_email_schedule() TO service_role;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'customer_email_schedule') THEN
    PERFORM cron.unschedule('customer_email_schedule');
  END IF;
  PERFORM cron.schedule(
    'customer_email_schedule',
    '*/10 * * * *',
    'SELECT public.run_customer_email_schedule();'
  );
END $$;
