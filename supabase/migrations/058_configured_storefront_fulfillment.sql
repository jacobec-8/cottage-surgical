-- 058 — Storefront fulfillment follows the configured item/location settings.
--
-- Storefront orders are requests which staff approve. A zero on-hand count must
-- not silently remove an otherwise configured pickup or delivery option from
-- checkout; serialized stock is still checked atomically by
-- confirm_rental_request() when staff approves the request.
...
BEGIN
  ...
  IF NEW.is_active AND NEW.pickup_enabled THEN
    ...
    JOIN public.pickup_locations l ON l.id = li.location_id
    WHERE li.equipment_item_id = NEW.id AND li.pickup_enabled AND l.is_active
...
-- Keep location activation/deactivation in sync without exposing stock counts.
...
DROP TRIGGER IF EXISTS sync_storefront_pickup_locations_for_location ON public.pickup_locations;
CREATE TRIGGER sync_storefront_pickup_locations_for_location
  AFTER UPDATE OF is_active ON public.pickup_locations
  FOR EACH ROW EXECUTE FUNCTION public.sync_storefront_pickup_locations_for_location();

-- Backfill every active, pickup-enabled item/location assignment, including
-- locations whose current count is zero.
INSERT INTO public.equipment_item_pickup_locations(equipment_item_id, pickup_location_id)
SELECT li.equipment_item_id, li.location_id
FROM public.equipment_location_inventory li
JOIN public.equipment_items i ON i.id = li.equipment_item_id
JOIN public.pickup_locations l ON l.id = li.location_id
WHERE li.pickup_enabled AND i.is_active AND i.pickup_enabled AND l.is_active
ON CONFLICT DO NOTHING;
...
-- Resolve fulfillment from explicit configuration. Quantity is deliberately
-- checked later, during staff approval, so customers can submit a request even
-- when the current on-hand count is zero.
